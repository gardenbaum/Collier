...
pub fn export_ts_bindings() {
    generate_bindings()
        // `BdError::Timeout::seconds` is `u64`. Specta-typescript >=0.0.12
        // forbids exporting BigInt-style primitives by default (would error
        // out the export and break CI). Timeouts in this app are small
        // (10s runner, 2s lock), so casting them to a TS `number` is safe
        // and keeps the bridge as a plain `number` on the TS side. In
        // tauri-specta >=2.0.0-rc.25 this knob moved from
        // `Typescript::bigint(BigIntExportBehavior::Number)` to the builder.
